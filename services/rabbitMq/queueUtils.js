...
class queuesUtil {
    ...
    async consumeExchange(exchange = EXCHANGE_NAME, exchangeType = EXCHANGE_TYPE, bindingKey = "", exchangeOptions = amqplib.Options.AssertExchange, consumeOptions = amqplib.Options.Consume, callback = () => {}) {
        return new Promise(async (resolve, reject) => {
            try {
    ...
    async publishMessageToExchange(exchange = EXCHANGE_NAME, exchangeType = EXCHANGE_TYPE, routingKey = "", exchangeOptions = amqplib.Options.AssertExchange, publishOptions = amqplib.Options.Publish, message = {}) {
        return new Promise(async (resolve, reject) => {
            try {
                const connection = await this.connect();
                const channel = await connection.createChannel();
